...
import org.francho.android.zgzbus.data.ZgzbusDb.Favoritos;
...
public class FavoritosProvider extends ContentProvider {
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
        ...
        if (TextUtils.isEmpty(sortOrder)) {
            orderBy = ZgzbusDb.Favoritos.DEFAULT_SORT_ORDER;
        } else {
            orderBy = sortOrder;
        }
    ...
    public Uri insert(Uri uri, ContentValues initialValues) {
        ...
        if (values.containsKey(ZgzbusDb.Favoritos.DESCRIPCION) == false) {
            values.put(ZgzbusDb.Favoritos.DESCRIPCION, Resources.getSystem().getString(android.R.string.untitled) );
        }
        ...
        if (rowId > 0) {
            Uri noteUri = ContentUris.withAppendedId(ZgzbusDb.Favoritos.CONTENT_URI, rowId);
    ...
    public int delete(Uri uri, String where, String[] whereArgs) {
        ...
        case FAVORITOS_ID:
            ...
            count = db.delete(TABLA_FAVORITOS, Favoritos._ID + "=" + favoritosId
    ...
    public int update(Uri uri, ContentValues values, String where, String[] whereArgs) {
        ...
        case FAVORITOS_ID:
            ...
            count = db.update(TABLA_FAVORITOS, values, Favoritos._ID + "=" + favoritoId 
            		+ (!TextUtils.isEmpty(where) ? " AND (" + where + ')' : ""), whereArgs);
            break;
    ...
    static {
        ...
        sUriMatcher.addURI(ZgzbusDb.AUTHORITY, "favoritos/poste/#", FAVORITOS_POSTE);
        sUriMatcher.addURI(ZgzbusDb.AUTHORITY, "favoritos/#", FAVORITOS_ID);
        sUriMatcher.addURI(ZgzbusDb.AUTHORITY, "favoritos", FAVORITOS);
        
        //sUriMatcher.addURI(ZgzbusDb.AUTHORITY, "live_folders/notes", LIVE_FOLDER_NOTES);
    ...
    /**
     * Clase de ayuda para abrir, crear y "upgradear" el fichero de base de datos
     */
    private static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        /**
         * Si todav’a no se ha creado la BBDD, la creamos y cargamos la estructura
         */
        @Override
        public void onCreate(SQLiteDatabase db) {
        	if(db.isReadOnly()) { db=getWritableDatabase(); }
            db.execSQL("CREATE TABLE " + TABLA_FAVORITOS + " (" +
            		Favoritos._ID + " INTEGER PRIMARY KEY AUTOINCREMENT," +
            		Favoritos.POSTE + " INTEGER," +            		
            		Favoritos.TITULO + " TEXT," +
            		Favoritos.DESCRIPCION + " TEXT"+
            		");");            	            
        }
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        	if(db.isReadOnly()) { db=getWritableDatabase(); }
            Log.w("FavoritosProvider", "Upgrading database from version " + oldVersion + " to "
