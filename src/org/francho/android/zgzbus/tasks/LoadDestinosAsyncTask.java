 ...
 * 
 */
public class LoadDestinosAsyncTask extends AsyncTask<String, Void, BusLinea> {
	 ...
	 * 
	 */
	public interface LoadDestinosAsyncTaskResponder {
		public void destinosLoaded(BusLinea bus);
	}

	private LoadDestinosAsyncTaskResponder responder;
	 ...
	 * @param responder
	 */
	public LoadDestinosAsyncTask(LoadDestinosAsyncTaskResponder responder) {
	...
	protected BusLinea doInBackground(String... linea) {
		...
		try {
			busLinea = Tuzsa.getDestinos(linea[0]);
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	protected void onPostExecute(BusLinea result) {
		if (responder != null) {
			responder.destinosLoaded(result);
		}
	}
